...
GulpTool.prototype.getTaskUse = function use(mode) {
  let fn = (done) => {
...
GulpTool.prototype.getTaskMkdir = function getTaskMkdir(pathPart) {
  let fn = async () => {
    let distPath = this.conf.dist;
    await fsPromises.mkdir(path.join(distPath, pathPart), {recursive: true});
  };
  Reflect.defineProperty(fn, 'name', {value: `mkdir_${pathPart}`});
  return fn;
};
...
GulpTool.prototype.getTaskLinkNodeModules = function getTaskLinkNodeModules() {
  let fn = this.getTaskLinks([
    {src: 'node_modules', to: 'node_modules'},
  ]);
  Reflect.defineProperty(fn, 'name', {value: `link_node_modules`});
  return fn;
};
